defmodule HomeHubWeb.Live.Screensaver do
  @moduledoc """
  Provides screensaver functionality for LiveView pages.

  ## Usage

  Add `use HomeHubWeb.Live.Screensaver` to your LiveView module,
  then call the helpers in your mount/render functions:

      defmodule MyLive do
        use HomeHubWeb, :live_view
        use HomeHubWeb.Live.Screensaver
  ...
  defmacro __using__(_opts) do
    quote do
      import HomeHubWeb.Live.Screensaver,
        only: [init_screensaver: 1, screensaver_events: 0]

      def handle_event("user_activity", _params, socket) do
        {:noreply, HomeHubWeb.Live.Screensaver.handle_user_activity(socket)}
      end

      def handle_info(:check_idle, socket) do
        {:noreply, HomeHubWeb.Live.Screensaver.handle_idle_check(socket)}
      end
    end
  end
